...
pub fn execute_program(alloc: std.mem.Allocator, program: []u8, mem: []u8, mbuff: []u8) !u64 {
    ...
    while (pc * ebpf.INSN_SIZE < program.len){
        ...
        // load ix
        const ix = ebpf.Instruction{
            .op = program[ebpf.INSN_SIZE * pc],
            .dst = program[ebpf.INSN_SIZE * pc + 1] & 0x0f,
            .src = (program[ebpf.INSN_SIZE * pc + 1] & 0x0f) >> 4,
            .offset = std.mem.readInt(u16,program[ebpf.INSN_SIZE + 2..]),
            .imm = std.mem.readInt(i32,program[ebpf.INSN_SIZE + 4..])
    };
        pc += 1;
